...
class TestConfigurationTypesEndpoints:
    ...
    async def test_unauthenticated_create_configuration_type(self, client: AsyncClient):
        ...
        response = await client.post(
            f"/api/organizations/{org_id}/configuration-types", json={"name": "Server"}
        )
        assert response.status_code == 401
...
class TestConfigurationStatusesEndpoints:
    ...
    async def test_unauthenticated_create_configuration_status(self, client: AsyncClient):
        ...
        org_id = uuid4()
        response = await client.post(
            f"/api/organizations/{org_id}/configuration-statuses", json={"name": "Active"}
        )
        assert response.status_code == 401
    ...
    async def test_unauthenticated_get_configuration_status(self, client: AsyncClient):
        ...
        response = await client.get(
            f"/api/organizations/{org_id}/configuration-statuses/{status_id}"
        )
        assert response.status_code == 401
...
class TestConfigurationsEndpoints:
    ...
    async def test_unauthenticated_create_configuration(self, client: AsyncClient):
        ...
        org_id = uuid4()
        response = await client.post(
            f"/api/organizations/{org_id}/configurations", json={"name": "web-server-01"}
        )
        assert response.status_code == 401
...
class TestConfigurationTypeWithAuth:
    ...
    async def test_list_configuration_types_empty(self, client: AsyncClient):
        ...
        with (
            patch("src.core.auth.get_current_active_user", return_value=mock_user),
            patch("src.core.auth.get_current_user", return_value=mock_user),
            patch("src.core.auth.get_current_user_optional", return_value=mock_user),
            patch("src.routers.configuration_types.ConfigurationTypeRepository") as mock_type_repo,
        ):
...
class TestConfigurationWithAuth:
    ...
    async def test_list_configurations_empty(self, client: AsyncClient):
        ...
        with (
            ...
            patch("src.core.auth.get_current_user", return_value=mock_user),
            patch("src.core.auth.get_current_user_optional", return_value=mock_user),
            patch("src.routers.configurations.ConfigurationRepository") as mock_config_repo,
        ):
            mock_config_repo.return_value.get_paginated_by_org = AsyncMock(return_value=([], 0))
